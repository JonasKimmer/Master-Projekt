...
Die Cluster werden im Kern von zwei Merkmalen getragen, `link_count` und `dom_nodes`. Das passt zum Befund von Ivory und Hearst (2002), dass quantitative Struktur- und Linkmerkmale prädiktiv für die wahrgenommene Seitenqualität sind. Der eigentliche methodische Ertrag liegt jedoch daneben: Zwei unter MDI relevant erscheinende Merkmale erweisen sich bei genauerer Prüfung als wertlos (4.3), eine unreflektierte Feature-Importance-Analyse hätte hier in die Irre geführt. Die Dekorrelations-Analyse (4.4) präzisiert das Bild. Der Informationsgewinn der sechs Merkmale ist real, aber geringer als ihre Redundanz, denn drei von sechs messen im Wesentlichen denselben „Umfang“. Ein Merkmalsset ohne die beiden redundanten Umfangs-Indikatoren wäre darum der Ausgangspunkt für ähnliche Analysen, mit dem Vorbehalt aus 4.4, dass gerade diese Vier-Merkmale-Lösung die extremste Seite als Single-Page-Cluster isoliert. Die Empfehlung bleibt damit vorläufig, bis sie an einem größeren Seitenkorpus geprüft ist.

Ein zweiter Befund betrifft die Struktur der Komplexität selbst. Das Profil von wiesbaden.de, das die geringste Knotenzahl (668) mit der tiefsten DOM-Verschachtelung (18,6) verbindet, zeigt, dass Komplexität kein eindimensionales Konstrukt ist. Butkiewicz et al. (2011) argumentieren in dieselbe Richtung und kombinieren mehrere unabhängige Metrikfamilien mit einem Außenkriterium. Genau diese Kombination wurde hier versuchsweise umgesetzt, ihr Außenkriterium geriet allerdings zu grob, um Strukturunterschiede aufzulösen. Dass ausgerechnet `link_count` als Einzelmetrik am tragfähigsten bleibt, fügt sich schließlich zu Chens (2018) Outdegree-Ansatz.

### 5.2 Grenzen und Einschränkungen

Die beiden Erfassungslücken aus 3.1 unterscheiden sich in der Ursache, nicht nur im Ergebnis, nämlich bei bpb.de ein Rendering-Problem (clientseitiges Nachladen), bei wiesbaden.de ein Scope-Problem (Formular außerhalb der gecrawlten Domain). Für die Praxis folgt daraus, dass ein Crawler-Tool eine leere Merkmalsauszählung nicht kommentarlos als Nullwert ausgeben sollte, sondern die wahrscheinliche Ursache mit ausweisen müsste.

**Stichprobe und Konfundierung:** Mit nur 3 Websites und 47 Beobachtungen, die zu drei Websites gehören statt unabhängig zu sein (ARI = 0,243), lässt sich nicht sicher zwischen generalisierbarem Seitentyp und individueller Website-Eigenheit trennen. Die Auswahl nach öffentlicher Zugänglichkeit und unauffälliger robots.txt ist zudem eine Positivselektion, die restriktiver geschützte, oft strukturell komplexere Websites ausschließt. Ob die Cluster-Typen auf andere Websitegattungen wie E-Commerce oder Social Media übertragbar sind, bleibt damit offen.

**Externes Kriterium:** Das einzige geprüfte externe Kriterium (4.5) bestätigt weder Clusterzugehörigkeit noch die wichtigsten Merkmale, und es deckt nur einen Ausschnitt möglicher Außenkriterien ab. Insbesondere die *wahrgenommene* Komplexität, die Tuch et al. (2012) und Miniukovich und De Angeli (2014) als eigenständige, verhaltensrelevante Dimension belegen, wurde nicht erhoben. Ob die strukturellen Merkmale abbilden, was Nutzende unter Komplexität verstehen, bleibt damit offen.

**Rechtlicher Rahmen:** Die robots.txt-Konformität (3.1) deckt nur die technische Zugriffsebene ab. Nutzungsbedingungen der jeweiligen Website können automatisiertes Crawling unabhängig davon einschränken. Für die hier gecrawlten drei öffentlichen Informations- und Verwaltungsportale wurden ausschließlich frei zugängliche, nicht personenbezogene Inhalte zu Forschungszwecken erfasst, eine kommerzielle Nutzung war nicht Gegenstand dieser Arbeit.
